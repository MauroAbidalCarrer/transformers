...
def eval_model(model: nn.Module, x: Tensor, y_true: Tensor) -> dict[str, float]:
    with torch.no_grad():
        ...
        y_pred = y_pred.reshape(BATCH_SIZE * ATTENTION_WINDOW_SIZE, N_EMBEDING_DIMS) # (batch size * window size, n embeding dims)
        y_true = y_true.reshape(BATCH_SIZE * ATTENTION_WINDOW_SIZE, N_EMBEDING_DIMS)
        return {
            "loss": F.cross_entropy(y_pred, y_true),
            "accuracy": torch.argmax(y_pred, dim=1) == torch.argmax(y_true, dim=1),
        }
...
class MaskedAttentionHead(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # x.shape = batch size, attention window size, embeding size
        keys: Tensor = self.keys_projection(x)
        queries = self.queries_projection(x)
        values = self.values_projection(x)
        weights = queries @ keys.swapaxes(1, 2)
        weights /= sqrt(self.head_size)
        weights = torch.masked_fill(weights, self.mask == 0, float('-inf'))
...
class GPT(nn.Module):
    def __init__(self):
        ...
        print("token_positions:", self.token_positions.shape)
...
print(sum(p.numel() for p in model.parameters())/1e6, 'mode parameters')
...
for iter in range(N_TRAINING_STEPS):
    ...
    if iter % LOGGING_INTERVAL == 0 or iter == N_TRAINING_STEPS - 1:
        ...
        print(f"step {iter}: train loss {train_metrics['loss']:.4f}, val loss {train_metrics['accuracy']:.4f}")
